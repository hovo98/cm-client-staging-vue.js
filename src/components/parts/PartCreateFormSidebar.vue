<template>
  <div class="crd-sidebar-holder">
    <div class="crd-sidebar">
      <h2 class="dash-title crd-sidebar--title">Create a Deal</h2>
      <ul class="crd-list">
        <li
        ...
        >
          <div v-if="showLinks">
            <a
              v-if="listItem.stepStatus === true"
              href="javascript:;"
              class="crd-sidebar__step-inner"
              @click="linkSideBar('/create-deal/' + listItem.reference + urlID, listItem.reference)"
            >
          ...
          <ul
            v-if="listItem.subList && listItem.openSidebar"
            class="crd-sublist"
          >
            <li
              v-for="subListItem in listItem.subList"
              :key="subListItem.id"
              :class="['crd-sublist__step', subListItem.stepStatus ? 'crd-sidebar__step-passed' : null, , subListItem.currentStep ? 'active-sublist': '']"
            >
              <a
                v-if="subListItem.stepStatus === true"
                class="crd-sidebar__step-inner crd-sidebar__step-inner-cursor"
                @click="linkSideBar('/create-deal/' + subListItem.reference + urlID, subListItem.reference)"
              >
                <SvgIcon
                  v-if="subListItem.stepStatus"
                  class="crd-sublist__step-status crd-sidebar__step-status"
                  icon-name="ico-check"
                />
                <span class="crb-step-title">{{ subListItem.title }}</span>
              </a>
              <div
                v-else
                class="crd-sidebar__step-inner"
...
<style lang="scss" scoped>
    .crd-sidebar__step-inner-cursor{
        cursor: pointer;
    }
    .crd-sidebar-holder {
        padding-left: 28px;
