<template>
  <div
    :class="[
      'page-dashboard page-individual-deal-broker',
  ...
  >
    ...
    <BlockBlockList
      ...
      :termsheet_status="termsheet_status"
      :titles="'broker'"
      :flag="'broker-broker'"
      :chat-name="chatName"
      :is-open-chat="isOpenChat"
      :is-deal-editable="isDealEditable"
      :until_next_edit="until_next_edit"
      @openOrCloseChat="openOrCloseChat"
    />
    <BlockIndividualDealBrokerTable
      v-if="quotes && quotes.length > 0 && deal"
      :quotes="quotes"
      :limit-reached="deal.quote_limit_reached"
    />
    <div class="individualDealBroker__wrapper">
      <div
      ...
      >
        ...
        <div
          v-if="deal.sponsor.sponsorInfo[0].name !== ''"
          class="col-xl-6"
        >
    ...
    <div class="bottom-chat-messages-wrapper">
      <button
        :class="chat && chat.rooms.length === 0 && quotes && quotes.length === 0 ? 'open-chat-btn disabled' : 'open-chat-btn'"
        @click="openOrCloseChat(!isOpenChat)"
      >
        <SvgIcon
          class="open-chat-icon"
          icon-name="ico-message-rounded"
        /> Messages<span v-if="chatCountNewMsg > 0"> <span class="open-chat-btn-message-indicator">({{ chatCountNewMsg }})</span></span><span
          v-if="chat && chat.rooms.length === 0 && quotes && quotes.length === 0"
          class="chat-btn-tooltip"
        >You don't have any messages for this deal yet</span>
      </button>
    </div>
    <ChatIndex
      v-if="deal && isOpenChat && chat && quotes"
      :deal-id="deal.id"
      :room-id="roomId"
      @openOrCloseChat="openOrCloseChat"
      @needSubscription="handleNeedSubscription"
    />
...
import BlockIndividualDealBrokerTable from "@/components/blocks/BlockIndividualDealBrokerTable.vue"
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      chatName: '',
      isOpenChat: false,
      termsheet_status: false,
      page: "page-create-deal",
      blockPreloader: true,
      isDealEditable: false,
      until_next_edit: null,
  ...
  async beforeMount() {
    ...
    try {
      ...
      const deal = await this.$store.dispatch("getDealByIdFilter", {
        id: this.$route.params.id,
      })
      //Sending this to BlockNlockList
      this.isDealEditable = deal.data.data.dealObject.deal_editable
      this.until_next_edit = deal.data.data.dealObject.until_next_edit

      await this.$store.dispatch("initialChatBroker", this.deal.id)
      if(this.chat.rooms && this.chat.rooms.length > 0) {
        let flag = false
      ...
      this.chatName = deal.data.data.dealObject.name

      if (deal.data.data.dealObject.status) {
        await this.$store.dispatch("brokerQuoteByDealId", {
          id: this.$route.params.id,
          showLenderName: !!this.subscription,
        })
        this.termsheet_status = deal.data.data.dealObject.termsheet_status
        this.setLogic()
  ...
  methods: {
    ...
    openOrCloseChat(data) {
      if(this.chat.rooms !== null || this.quotes !== null) {
        if(this.chat.rooms.length === 0 && this.quotes.length === 0)
          return
      } else {
        return
      }
...
.individualDealBroker__wrapper {
  padding: 0 52px 30px 42px;
...
.open-chat-btn {
    ...
    .bottom-chat-messages-wrapper {
      ...
      @include mq(880px) {
        .open-chat-btn.disabled .chat-btn-tooltip {
          top: -45px;
          left: 50%;
